...
mout <- mplusModeler(m17_mplus, dataout="flanker_mplus.dat", 
                        modelout="m17_mplus.inp", run=TRUE, Mplus_command = mpluscmd)
...
mout <- mplusModeler(m17_mplus_noruntrial, dataout="flanker_mplus.dat", 
                        modelout="m17_mplus_noruntrial.inp", run=TRUE, Mplus_command = mpluscmd)
...
mout <- mplusModeler(m17_mplus_fixedruntrial, dataout="flanker_mplus.dat", 
                        modelout="m17_mplus_fixedruntrial.inp", run=TRUE, Mplus_command = mpluscmd)
...
Between-persons CFA suggests that P1 cross-loads on inhib. Including this cross-loading makes the fit good enough IMO, while without it, CFI is < .9. Do our effects here hold if we allow that?

Indeed, the ANTAG effects are consistent, even slightly stronger. So, no problems with whether P1 is a secondary indicator of inhib. But, the stickiness of RT `PREVSLO ON INHIB` becomes non-significant, p = .10.

```{r antag_mod, cache=TRUE}
AC_msem_mod <- mplusObject(
  TITLE="Antagonism and Disinhibition BSEM CFA 3+3 Parcels, Flanker RT Mod",
  DEFINE="
    p1=MEAN(MISTRUST MPS_alR);
    p2=MEAN(PROPER MPS_tdR);
    p3=MEAN(MPS_clR IMPUL);
  ...
  %BETWEEN%
    ...
    INHIB BY P1; !modification index cross-loading for fit
...
mres <- mplusModeler(AC_msem_mod, modelout = file.path("AC_flanker_msem_parcel_mod.inp"), 
                     hashfilename = FALSE, run=TRUE,
                     Mplus_command = mpluscmd)
```
...
mres <- mplusModeler(Dom_msem, modelout = file.path("Dom_modela_flanker_rt_msem.inp"), 
                     hashfilename = FALSE, run=TRUE,
                     Mplus_command = mpluscmd)
```
```{r}
...
# Add agentic extraversion to Antag + Inhib model
...
AC_msem_mod <- mplusObject(
  ...
  %BETWEEN%
    ...
    !two cross-loadings for Inhib suggested by MLR
    INHIB BY ENTITL;
    INHIB BY P1;
    ...
    ! trait moderates flanker performance -- incongruency effect and stickiness of RT
    condslo ON antag inhib agentic;
    prevslo ON antag inhib agentic;
    errslo ON antag inhib agentic; 

    !average RT predicted by traits    
    rt_inv ON antag inhib agentic;
  ",
  PLOT = "TYPE = PLOT2;",
  OUTPUT ="
    TECH1 TECH8 STANDARDIZED CINTERVAL;
...
mres <- mplusModeler(AC_msem_mod, modelout = file.path("AC_flanker_msem_agent.inp"), 
                     hashfilename = FALSE, run=TRUE,
                     Mplus_command = mpluscmd)
```
```{r}
...
# Add affiliative extraversion to antag + inhib model
...
Dom_msem <- mplusObject(
  TITLE="Antag, Inhib, Affiliat MSEM Flanker RT",
  DEFINE="
    p1=MEAN(MISTRUST MPS_alR);
    p2=MEAN(PROPER MPS_tdR);
    p3=MEAN(MPS_clR IMPUL);
  ...
  %BETWEEN%
    ...
    antag inhib affiliat WITH antag inhib affiliat;
    ...
    ! trait moderates flanker performance -- incongruency effect and stickiness of RT
    condslo ON antag inhib affiliat;
    prevslo ON antag inhib affiliat;
    errslo ON antag inhib affiliat;
    
    rt_inv ON antag inhib affiliat;
  ",
  PLOT = "TYPE = PLOT2;",
  OUTPUT ="
    TECH1 TECH8 CINTERVAL STANDARDIZED;
...
mres <- mplusModeler(Dom_msem, modelout = file.path("AC_flanker_msem_affil.inp"), 
                     hashfilename = FALSE, run=TRUE,
                     Mplus_command = mpluscmd)
```
```{r}
...
Combo_msem <- mplusObject(
  ...
  %BETWEEN%
    ...
    errslo ON antag inhib agentic affiliat; !highly non-significant
    
    rt_inv ON antag inhib agentic affiliat;
  ",
  PLOT = "TYPE = PLOT2;",
  OUTPUT ="
    TECH1 TECH8 CINTERVAL STANDARDIZED;
...
mres <- mplusModeler(Combo_msem, modelout = file.path("Combined_flanker_rt_msem.inp"), 
                     hashfilename = FALSE, run=TRUE,
                     Mplus_command = mpluscmd)
```
```{r}
