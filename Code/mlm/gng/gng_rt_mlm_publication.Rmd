...
## m8: re-introduce condition, now that trials from no-go is a random effect

```{r m8, cache=TRUE}
minfo[["m8"]] <- c(fixed="Condition, Trials from no-go", l2="Subject: Intercept, Trials from no-go")

m8 <- lmer(rt_log_trim_grp ~ cond + block_trial_z + (1 + block_trial_z | id),
           na.action = na.exclude, 
          data = gng_rt, control=lmerControl(optCtrl=list(maxfun=2e4))) #bump up iterations)
summary(m8)
# emmeans(m2, ~cond)
