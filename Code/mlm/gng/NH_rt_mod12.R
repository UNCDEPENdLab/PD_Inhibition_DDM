...
for_msem <- gng %>% mutate(id = subj_idx) %>% inner_join(tofactor, by="id") %>% ungroup() %>%  select(id, response, cond, stim, trial_z, rt_log_trim_grp, block_trial_z, prev_error, MPS_agR, AGG, MISTRUST, MPS_alR, MANIP,
                                                                                        PROPER, MPS_clR, IMPUL, MPS_tdR, MPS_acR, HDWK, K10, STAI) %>% 
...
mout <- mplusModeler(rt_with_mod_gng2,
                     modelout="rt_with_mod_gng2.inp", run=TRUE, Mplus_command = mpluscmd, hashfilename = FALSE)
