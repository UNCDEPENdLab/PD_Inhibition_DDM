...
flanker <- flanker %>%
  ...
  mutate(
    ...
    prev_error=factor(1-dplyr::lag(correct, 1, order_by=run_trial), levels=c(0,1), labels=c("no", "yes"))
  ) %>% ungroup()
...
https://www.flutterbys.com.au/stats/tut/tut8.3b.html

N.B. We are now analyzing only RTs on correct trials, not all RTs.
...
flanker <- flanker %>% filter(correct==1)
```

## m0: random intercept of subject, experiment conditions only
```{r m0, cache=TRUE}
m0 <- lme(rt_inv ~ cond*block, random = ~ 1 | id,
...
## m8a: use nesting structure, not pdDiag for per-block random intercept

As elsewhere, the nesting approach is ever so slightly better than a blocked pdDiag L2 structure. Still the same model, though, and more conceptually familiar.

```{r m8a, cache=TRUE}
#this model allows for heterogeneity in the L1 residuals by block, as well as the L2 random effects by block
m8a <- lme(rt_inv ~ trial_z + cond*block, 
          weights=varIdent(form=~1|block),
          random=~1|id/block,
          na.action = na.exclude,
          data = flanker, method='ML')
...
summary(m8a)
AICtab(m2, m4, m5, m6, m7, m8, m8a, sort=TRUE, delta=TRUE, weights=TRUE, logLik=TRUE, base=TRUE)
```
...
Note that this took about 6 hours to run on my machine, so be careful in knitting!

Here, we are using the 'distributional model' approachs in brms where the distribution of the level 1 residual variance is itself a variable to be fit by the model. This is described here: https://cran.r-project.org/web/packages/brms/vignettes/brms_distreg.html

```{r m30_bayes, cache=TRUE}
