...
gng <- mutate(gng, block_trial_z = scale(block_trial)) %>% filter(response == 1, stim == "Go") 
...
Following AMS's original script, look at each factor score separately in lmer framework and add personality trait as an l1 predictor.

```{r rt_mod_antagonism, cache=TRUE}
m12_antD <- lmer(rt_log_trim_grp ~  block_trial_z*trial_z*ANTAG + block_trial_z*trial_z*DISINH + (1 + block_trial_z + trial_z| id) + (1 + block_trial_z | id:cond),
                na.action = na.exclude, 
                data = gng, control=lmerControl(optCtrl=list(maxfun=2e4)), REML = FALSE) #bump up iterations)
summary(m12_antD)

test(emtrends(m12_antD, ~block_trial_z*trial_z, var = "ANTAG", at = list(block_trial_z = c(-2,-1,1, 2), trial_z = c(-2,-1,1,2))))

library(interactions)


mb <- mean(gng$block_trial_z)
sb <- sd(gng$block_trial_z)
btris <- c((mb-2*sb), (mb-sb), mb, (mb+sb), (mb+2*sb))
mt <- mean(gng$trial_z)
st <- sd(gng$trial_z)
tris <- c((mt-2*st), (mt-st), mt, (mt+st), (mt+2*st))
labs <- c("-2SD", "-1SD", "Mean", "+1SD", "+2SD")
labs_t <- c("Trial -2SD", "Trial -1SD", "Trial Mean", "Trial +1SD", "Trial +2SD")

ant_plot <- interact_plot(m12_ant, pred = "ANTAG", 
                          modx = "block_trial_z", #modx.values = btris, modx.labels = labs,
                          mod2 = "trial_z")#,#mod2.values = tris, mod2.labels = labs_t,
                          # interval = TRUE)
ant_plot <- ant_plot  +  scale_y_continuous(breaks = c(2.50, 2.52, 2.54, 2.56, 2.58), limits = c(2.50, 2.59))
plot(ant_plot)
```
Looks like preliminarily, high antagonism folks early in the task, on trials that are far from the last encountered no-go are much faster than folks who are low in antagonism. So, early in the task it looks like subjects have a tendency to speed up at they get further away from a no-go, which gets learned over the course of the task but could lead to more errors early in the task. Looks like a sort of "initial recklessness bias".  


```{r rt_mod_disinh, cache=TRUE}
m12_dis <- lmer(rt_log_trim_grp ~  block_trial_z*trial_z*DISINH + (1 + block_trial_z + trial_z| id) + (1 + block_trial_z | id:cond),
                na.action = na.exclude, 
                data = gng, control=lmerControl(optCtrl=list(maxfun=2e4)), REML = FALSE) #bump up iterations)
summary(m12_dis)

#interesting, some moderate evidence of a 3-way ixn in disinh

test(emtrends(m12_dis, ~block_trial_z*trial_z, var = "DISINH", at = list(block_trial_z = c(-2,-1,1,2), trial_z = c(-2,-1,1,2))))
# test(emtrends(m12_dis, ~block_trial_z*trial_z, var = "DISINHIBITION_F", at = list(block_trial_z = c(-2,-1,1,2), trial_z = c(-2,-1,1,2))))
library(interactions)

dis_plot <- interact_plot(m12_dis, pred = "DISINH", 
                          # outcome.scale = "10^",
                          modx = "block_trial_z", #modx.values = btris, modx.labels = labs,
                          mod2 = "trial_z")#, #mod2.values = tris, mod2.labels = labs_t,
#                           interval = TRUE
...
dis_plot <- dis_plot +  scale_y_continuous(breaks = c(2.50, 2.52, 2.54, 2.56, 2.58), limits = c(2.50, 2.59))

plot(dis_plot)
cowplot::plot_grid(ant_plot, dis_plot, ncol = 2)
```
