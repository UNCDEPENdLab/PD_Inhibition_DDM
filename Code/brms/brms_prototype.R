...
pdddm_home <- "~/github_repos/PD_Inhibition_DDM"
...
vreg_traces_summary <- vreg_traces %>% group_by(subj, stat) %>% 
  dplyr::summarise(mean = mean(value),se = summary(as.mcmc(value))$statistics[["Time-series SE"]])  %>% 
...
pdata <- read.csv(file.path(pdddm_home, "Data", "summarydat.csv")) %>% dplyr::select(Subject, MPQAggression) %>% dplyr::rename(subj = Subject)
# pdata <- read.csv(file.path(pdddm_home, "Data", "summarydat.csv")) %>% dplyr::select(Subject, starts_with("T_"), starts_with("MPQ")) %>% dplyr::rename(subj = Subject)
