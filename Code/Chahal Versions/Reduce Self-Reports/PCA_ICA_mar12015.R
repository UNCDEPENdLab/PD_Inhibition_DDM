...
##
fa.diagram1<-fa.diagram(fa1$loadings, sort=TRUE, simple=TRUE, digits=1, e.size=.05, rsize=1.5) 
fa.diagram2<-fa.diagram(fa1$loadings, digits=1, e.size=.05,cut=.3, rsize=.1, side=2, cex=NULL)
##
dfcor<-cor(df)
fa.promax <- fa(dfcor, nfactors=8, rotate="promax", fm="pa")
...
library("Rgraphviz")
fa.rgraph(fa1$loadings,simple=TRUE,
