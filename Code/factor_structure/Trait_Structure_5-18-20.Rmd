...
snap <- get(load(file.path(basedir, "Data/preprocessed/SNAP_all_scored_final.RData"))) %>% filter(exclude_SNAP == 0) %>% select(subject, NEGTEMP:HDWK)
...
```{r echo = FALSE}
sr.fa.AC <- fa(select(self_reps, IMPUL, PROPER, HDWK, DISINH, MPS_acR, MPS_clR, MPS_tdR, MPS_agR, AGG, MPS_alR, MANIP, MISTRUST, MPS_haR), 2, rotate="oblimin", fm = "pa")
...
```{r echo = FALSE}
AC_fscores <- factor.scores(select(self_reps, IMPUL, PROPER, HDWK, DISINH, MPS_acR, MPS_clR, MPS_tdR, MPS_agR, AGG, MPS_alR, MANIP, MISTRUST, MPS_haR), sr.fa.AC$loadings, Phi = sr.fa.AC$Phi, method = "regression")
...
write.csv(fac_scores, "C:/Users/timot/Documents/GitHub/PD_Inhibition_DDM/Outputs/factor_structure/PDDDM_factor_scores_5-18-20.csv", row.names = F)
```
