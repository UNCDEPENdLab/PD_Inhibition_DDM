...
problem_accs <- summaries_con %>% filter(Subject_ACC_con <= .6) %>% arrange(Subject_ACC_con) %>% print()
...
```{r}
str(flank_cleaned)
flank_use <- flank_cleaned %>% select(Subject, TrialSlide_ACC, rt, Incongruent, CongruentBlock, ABBA) %>% group_by(Subject, ABBA) %>% mutate(prev_stim = lag(Incongruent,n = 1, default = 0), trial_block = seq(1,length(Subject), 1)) %>% group_by(Subject) %>% mutate(trial_sub = seq(1,length(Subject),1))%>% ungroup() %>% left_join(SNAPz, by = "Subject") 
